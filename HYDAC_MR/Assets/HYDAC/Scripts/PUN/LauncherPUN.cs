...
namespace HYDAC.Scripts.PUN
{
    ...
    /// that the master client has created, allowing players to join a networked session in game and launching into the main scene: Hydac_Factory.unity.
    /// </summary>
    public class LauncherPUN : MonoBehaviourPunCallbacks
    {
        #region Private and Public Attributes
        // Public Attributes
        public const string ROOM_NAME = "AssemblyView";    // Also accessed by RoomMgrPUN.cs

        // Private Attributes
        private const string _GAME_VERSION = "1";           // Set to 1 by default, unless we need to make breaking changes on a project that is Live.
        ...
        [SerializeField]
        private GameObject _ConnectionText;
        ...
        public void Connect()
        {
            ...
            else
            {
                ...
                PhotonNetwork.GameVersion = _GAME_VERSION;
        ...
        {
            // Check if we are wanting to connect (prevent looping if / when we disconnect from a room)
            if (_IsConnecting)
            {
                // Critical
