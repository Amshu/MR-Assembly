...
{
    /// <summary>
    /// This class is responsible for tracking the finger and wrist joints on the Hololens user's left and right hands and determining the 
    /// type of pose (normal, finger point, thumbs up) the user is making.
    /// </summary>
    public class FollowTrackedFingers : MonoBehaviour
    {
        ...
        private MixedRealityPose _Pose;
        ...
        private void Update()
        {
            ...
            if (HandJointUtils.TryGetJointPose(TrackedHandJoint.ThumbTip, Handedness.Left, out _Pose))
            {
                _FingerTipVectorsLeft[0] = _Pose.Position;
                //_FingerObjsLeft[0].GetComponent<Renderer>().enabled = true;
            }

            if (HandJointUtils.TryGetJointPose(TrackedHandJoint.IndexTip, Handedness.Left, out _Pose))
            {
                _FingerTipVectorsLeft[1] = _Pose.Position;
                //_FingerObjsLeft[1].GetComponent<Renderer>().enabled = true;
            }

            if (HandJointUtils.TryGetJointPose(TrackedHandJoint.MiddleTip, Handedness.Left, out _Pose))
            {
                _FingerTipVectorsLeft[2] = _Pose.Position;
                //_FingerObjsLeft[2].GetComponent<Renderer>().enabled = true;
            }

            if (HandJointUtils.TryGetJointPose(TrackedHandJoint.RingTip, Handedness.Left, out _Pose))
            {
                _FingerTipVectorsLeft[3] = _Pose.Position;
                //_FingerObjsLeft[3].GetComponent<Renderer>().enabled = true;
            }

            if (HandJointUtils.TryGetJointPose(TrackedHandJoint.PinkyTip, Handedness.Left, out _Pose))
            {
                _FingerTipVectorsLeft[4] = _Pose.Position;
                //_FingerObjsLeft[4].GetComponent<Renderer>().enabled = true;
            }

            if (HandJointUtils.TryGetJointPose(TrackedHandJoint.Wrist, Handedness.Left, out _Pose))
            {
                _WristLeftPosition = _Pose.Position;
                _WristLeftRotation = _Pose.Rotation;
            ...
            // RIGHT HAND
            // Attempt to track fingertips and wrist
            if (HandJointUtils.TryGetJointPose(TrackedHandJoint.ThumbTip, Handedness.Right, out _Pose))
            {
                _FingerTipVectorsRight[0] = _Pose.Position;
                //_FingerObjsRight[0].GetComponent<Renderer>().enabled = true;
            }

            if (HandJointUtils.TryGetJointPose(TrackedHandJoint.IndexTip, Handedness.Right, out _Pose))
            {
                _FingerTipVectorsRight[1] = _Pose.Position;
                //_FingerObjsRight[1].GetComponent<Renderer>().enabled = true;
            }

            if (HandJointUtils.TryGetJointPose(TrackedHandJoint.MiddleTip, Handedness.Right, out _Pose))
            {
                _FingerTipVectorsRight[2] = _Pose.Position;
                //_FingerObjsRight[2].GetComponent<Renderer>().enabled = true;
            }

            if (HandJointUtils.TryGetJointPose(TrackedHandJoint.RingTip, Handedness.Right, out _Pose))
            {
                _FingerTipVectorsRight[3] = _Pose.Position;
                //_FingerObjsRight[3].GetComponent<Renderer>().enabled = true;
            }

            if (HandJointUtils.TryGetJointPose(TrackedHandJoint.PinkyTip, Handedness.Right, out _Pose))
            {
                _FingerTipVectorsRight[4] = _Pose.Position;
                //_FingerObjsRight[4].GetComponent<Renderer>().enabled = true;
            }

            if (HandJointUtils.TryGetJointPose(TrackedHandJoint.Wrist, Handedness.Right, out _Pose))
            {
                _WristRightPosition = _Pose.Position;
                _WristRightRotation = _Pose.Rotation;
